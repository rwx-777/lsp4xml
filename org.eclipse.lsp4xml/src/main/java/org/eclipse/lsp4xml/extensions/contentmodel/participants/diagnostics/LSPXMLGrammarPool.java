...
import org.eclipse.lsp4xml.extensions.contentmodel.model.FileChangedTracker;
...
public class LSPXMLGrammarPool implements XMLGrammarPool {
	...
	/*
	 * <p> Retrieve the initial known set of grammars. This method is called by a
	 ...
	 * <code>org.apache.xerces.xni.grammars.XMLGrammarDescription</code> interface.
	 * 
	...
	/*
	 * <p> Return the final set of grammars that the validator ended up with. This
	 * method is called after the validation finishes. The application may then
	 * choose to cache some of the returned grammars.</p> <p>In this implementation,
	 * we make our choice based on whether this object is "locked"--that is, whether
	 * the application has instructed us not to accept any new grammars.</p>
	 *
	 * @param grammarType The type of the grammars being returned;
	 * 
	 * @param grammars An array containing the set of grammars being returned; order
	 * is not significant.
	 */
	@Override
	 ...
	 * corresponding to the given GrammarIdentifier from its cache. If it cannot do
	 * so it must return null; the parser will then call the EntityResolver. </p>
	 ...
	 * grammar and the target namespace to identify a Schema grammar.
	 *
	...
	/*
	 * <p> Sets this grammar pool to a "locked" state--i.e., no new grammars will be
	 * added until it is "unlocked".
	 */
	@Override
	...
	/*
	 * <p> Sets this grammar pool to an "unlocked" state--i.e., new grammars will be
	 * added when putGrammar or cacheGrammars are called.
	 */
	@Override
	...
	/*
	 * <p>This method clears the pool-i.e., removes references to all the grammars
	 * in it.</p>
	 */
	@Override
	public void clear() {
	...
	protected static final class Entry {
		...
		public Entry next;
		private FileChangedTracker tracker;
		...
		protected Entry(int hash, XMLGrammarDescription desc, Grammar grammar, Entry next) {
			...
			this.next = next;
			Path file = getFile(desc);
			if (file != null) {
				tracker = new FileChangedTracker(file);
			}
		}

		private static Path getFile(XMLGrammarDescription desc) {
			try {
				String fileURI = desc.getExpandedSystemId();
	 ...
	 * schema.getTriggeringComponent()); System.out.println("EnclosingElementName ="
